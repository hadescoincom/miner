...
#ifdef WIN32
		std::wstring sPathArg = hds::Utf8toUtf16(sz);
#else // WIN32
...
void hds::Crash::InstallHandler(const char* szLocation)
{
	if (szLocation)
	{
		std::wstring s = hds::Utf8toUtf16(szLocation);
		size_t nLen = s.size();
...
void hds::Crash::InstallHandler(const char*)
{
}

#endif // WIN32

void hds::Crash::Induce(Type type)
{
	switch (type)
	{
	case StlInvalid:
